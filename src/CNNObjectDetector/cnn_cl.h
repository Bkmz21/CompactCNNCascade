	...
	namespace CL
	{
		...
		private:
			...
			struct CNN
			{
				...
				std::vector<SIMD::Array_32f> subs_weight;
				std::vector<SIMD::Array_32f> subs_bias;
		...
		public:
			...
			void Init(std::string file_name, int, cl_device_id _devices, cl_context _context, cl_command_queue _queue);
...
#if 0
			void Init(ConvNeuralNetwork* _cnn, bool _max_pool = false, bool preprocessing = false);
			void SaveToBinaryFile(std::string file_name);
#endif
