...
namespace NeuralNetworksLib
{
	...
	namespace CL
	{
		void ConvNeuralNetwork::Init(std::string file_name, int index_output, cl_device_id _device, cl_context _context, cl_command_queue _queue)
		{
			...
			for (int i = 0; i < cnn.layer_count; ++i)
			{
				...
				{
					int t = j;
					if (i == 2 && j >= cnn.layer_buffer[i].map_count / 2)
					{
						t = cnn.layer_buffer[i].map_count - 1 - j % (cnn.layer_buffer[i].map_count / 2);
					}

					FB_READ(data_bin, cnn.conv_bias[i][t]);
					FB_READ(data_bin, cnn.leakyReLU_w1[i][t]);
					FB_READ(data_bin, cnn.leakyReLU_w2[i][t]);
					FB_READ(data_bin, cnn.bn_weight[i][t]);
					FB_READ(data_bin, cnn.bn_bias[i][t]);
				}

				CNNPP::set_conv_b_on_device(context, queue, cnn.conv_bias[i](), i + 1, cnn.layer_buffer[i].map_count);
				CNNPP::set_lrelu_w1_on_device(context, queue, cnn.leakyReLU_w1[i](), i + 1, cnn.layer_buffer[i].map_count);
				CNNPP::set_lrelu_w2_on_device(context, queue, cnn.leakyReLU_w2[i](), i + 1, cnn.layer_buffer[i].map_count);
				//CNNPP::set_bn_w_on_device(context, queue, cnn.bn_weight[i](), i + 1, cnn.layer_buffer[i].map_count);
				CNNPP::set_bn_b_on_device(context, queue, cnn.bn_bias[i](), i + 1, cnn.layer_buffer[i].map_count);
			}
			...
			{
				printf("[CL::CNN] This configuration cnn model_types is not supported!\n");
				Clear();
				return;
			}

			cnn.snn_hl_weight = SIMD::Array_32f(cnn.snn_hl_size * cnn.snn_connect_count, ALIGN_SSE);
			for (int i = 0; i < cnn.snn_hl_size; ++i)
			{
				for (int j = 0; j < cnn.snn_connect_count; ++j)
				{
					FB_READ(data_bin, cnn.snn_hl_weight[i * cnn.snn_connect_count + j]);
				}
			}
			CNNPP::set_hl_w_on_device(context, queue, cnn.snn_hl_weight(), cnn.snn_hl_weight.size);
			...
			CNNPP::set_hl_b_on_device(context, queue, cnn.snn_hl_bias(), cnn.snn_hl_bias.size);
			...
			CNNPP::set_hl_tanh_w_on_device(context, queue, cnn.snn_hl_tanh_w(), cnn.snn_hl_tanh_w.size);
			...
			CNNPP::set_hl_bn_w_on_device(context, queue, cnn.snn_hl_bn_weight(), cnn.snn_hl_bn_weight.size);
			CNNPP::set_hl_bn_b_on_device(context, queue, cnn.snn_hl_bn_bias(), cnn.snn_hl_bn_bias.size);

			FB_READ(data_bin, cnn.snn_ol_neuron_count);
			cnn.snn_ol_weight = SIMD::Array_32f(cnn.snn_ol_neuron_count * cnn.snn_hl_size, ALIGN_SSE);
			for (int i = 0; i < cnn.snn_ol_neuron_count; ++i)
			{
				for (int j = 0; j < cnn.snn_hl_size; ++j)
				{
					FB_READ(data_bin, cnn.snn_ol_weight[i * cnn.snn_hl_size + j]);
				}
			}
			CNNPP::set_ol_w_on_device(context, queue, cnn.snn_ol_weight(), cnn.snn_ol_weight.size);
			...
			CNNPP::set_ol_b_on_device(context, queue, cnn.snn_ol_bias(), cnn.snn_ol_bias.size);

			FB_READ(data_bin, cnn.snn_ol_tanh_w);
			CNNPP::set_ol_tanh_w_on_device(context, queue, &cnn.snn_ol_tanh_w);
			...
			CNNPP::set_ol_scale_on_device(context, queue, &af_scale);
			CNNPP::set_index_output_on_device(context, queue, cnn.index_output);
		}
		void ConvNeuralNetwork::AllocateMemory(Size size)
		{
			...
			for (int i = 0; i < cnn.layer_buffer[0].map_count; ++i)
			{
				...
				CNNPP::set_src_surf(&cnn.layer_buffer[0].buffer[i], 2, i);
			}
			...
			for (int i = 0; i < cnn.layer_buffer[1].map_count; ++i)
			{
				...
				CNNPP::set_src_surf(&cnn.layer_buffer[1].buffer[i], 3, i);
			}
			...
			for (int i = 0; i < cnn.layer_buffer[2].map_count; ++i)
			{
				...
				CNNPP::set_src_surf(&cnn.layer_buffer[2].buffer[i], 4, i);
			}
		...
		void ConvNeuralNetwork::Forward(Image_32f* response_map, Image_32f* image)
		{
			...
			CNNPP::set_src_surf(image, 1, 0);
			CNNPP::run_L1(cnn.conv_l1.ROI, block_size, queue);
			...
			//	int count1 = 0;
			//	double d1 = 0;
			//	for (int y = 0; y < cnn.conv_l1.ROI.rows; ++y)
			//	{
			//		for (int x = 0; x < cnn.conv_l1.ROI.cols; ++x)
			//		{
			//			double d = cnn.layer_buffer[0].buffer[i].dataHost[y * cnn.layer_buffer[0].size.cols + x];
			//			printf("%f ", d);
			//		}
			//		printf("\n");
			//	}
			...
			CNNPP::run_L2(cnn.conv_l2.ROI, block_size, queue);
			...
			//	int count1 = 0;
			//	double d1 = 0;
			//	for (int y = 0; y < cnn.conv_l2.ROI.rows; ++y)
			//	{
			//		for (int x = 0; x < cnn.conv_l2.ROI.cols; ++x)
			//		{
			//			double d = cnn.layer_buffer[1].buffer[i].dataHost[y * cnn.layer_buffer[1].size.cols + x];
			//			printf("%f ", d);
			//		}
			//		printf("\n");
			//	}
			...
			CNNPP::run_L3(cnn.conv_l3.ROI, block_size, queue);

			//for (int i = 0; i < cnn.layer_buffer[2].map_count; ++i)
			//{
			//	cnn.layer_buffer[2].buffer[i].updateDataHost();
			//	int count1 = 0;
			//	double d1 = 0;
			//	for (int y = 0; y < cnn.conv_l3.ROI.rows; ++y)
			//	{
			//		for (int x = 0; x < cnn.conv_l3.ROI.cols; ++x)
			//		{
			//			double d = cnn.layer_buffer[2].buffer[i].dataHost[y * cnn.layer_buffer[2].size.cols + x];
			//			printf("%f ", d);
			//		}
			//		printf("\n");
			//	}
			...
			CNNPP::run_HL(cnn.conv_l3.ROI, block_size, queue);
